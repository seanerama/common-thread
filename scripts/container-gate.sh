...
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --record-file "$proof_dir/person.json"
...
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management on --context-notes on --party-directory on --record-file "$proof_dir/managed-person.json"
export PEOPLE_MANAGEMENT_ENABLED=false CONTEXT_NOTES_ENABLED=false PARTY_DIRECTORY_ENABLED=false
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management off --context-notes off --party-directory off --read-file "$proof_dir/managed-person.json"
export PEOPLE_MANAGEMENT_ENABLED=false CONTEXT_NOTES_ENABLED=false PARTY_DIRECTORY_ENABLED=true
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management off --context-notes off --party-directory on --read-file "$proof_dir/managed-person.json"
export PEOPLE_MANAGEMENT_ENABLED=true CONTEXT_NOTES_ENABLED=true PARTY_DIRECTORY_ENABLED=true
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management on --context-notes on --party-directory on --read-file "$proof_dir/managed-person.json"
