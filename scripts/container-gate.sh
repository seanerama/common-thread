...
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management on --context-notes on --party-directory on --relationships on --interactions on --commitments on --record-file "$proof_dir/managed-person.json"
export PEOPLE_MANAGEMENT_ENABLED=false CONTEXT_NOTES_ENABLED=false PARTY_DIRECTORY_ENABLED=false RELATIONSHIPS_ENABLED=false INTERACTIONS_ENABLED=false COMMITMENTS_ENABLED=false
docker compose up -d --force-recreate --wait app
...
export PEOPLE_MANAGEMENT_ENABLED=true CONTEXT_NOTES_ENABLED=true PARTY_DIRECTORY_ENABLED=true RELATIONSHIPS_ENABLED=true INTERACTIONS_ENABLED=true COMMITMENTS_ENABLED=true
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management on --context-notes on --party-directory on --relationships on --interactions on --commitments on --read-file "$proof_dir/managed-person.json"
