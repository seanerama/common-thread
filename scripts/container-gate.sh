...
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management on --context-notes on --record-file "$proof_dir/managed-person.json"
export PEOPLE_MANAGEMENT_ENABLED=false CONTEXT_NOTES_ENABLED=false
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management off --context-notes off --read-file "$proof_dir/managed-person.json"
export PEOPLE_MANAGEMENT_ENABLED=true CONTEXT_NOTES_ENABLED=true
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management on --context-notes on --read-file "$proof_dir/managed-person.json"
