...
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --relationships off --interactions off --commitments off --record-file "$proof_dir/person.json"
...
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management on --context-notes on --party-directory on --relationships on --interactions on --commitments on --person-overview on --record-file "$proof_dir/managed-person.json"
export PERSON_OVERVIEW_ENABLED=false
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management on --context-notes on --party-directory on --relationships on --interactions on --commitments on --person-overview off --read-file "$proof_dir/managed-person.json"
export PERSON_OVERVIEW_ENABLED=true
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management on --context-notes on --party-directory on --relationships on --interactions on --commitments on --person-overview on --read-file "$proof_dir/managed-person.json"
export PEOPLE_MANAGEMENT_ENABLED=false CONTEXT_NOTES_ENABLED=false PARTY_DIRECTORY_ENABLED=false RELATIONSHIPS_ENABLED=false INTERACTIONS_ENABLED=false COMMITMENTS_ENABLED=false PERSON_OVERVIEW_ENABLED=false
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management off --context-notes off --party-directory off --relationships off --interactions off --commitments off --read-file "$proof_dir/managed-person.json"
export INTERACTIONS_ENABLED=true
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management off --context-notes off --party-directory off --relationships off --interactions on --commitments off --read-file "$proof_dir/managed-person.json"
export INTERACTIONS_ENABLED=false
export COMMITMENTS_ENABLED=true
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management off --context-notes off --party-directory off --relationships off --interactions off --commitments on --read-file "$proof_dir/managed-person.json"
...
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management off --context-notes off --party-directory off --relationships on --interactions off --commitments off --read-file "$proof_dir/managed-person.json"
...
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management off --context-notes off --party-directory on --relationships off --interactions off --commitments off --read-file "$proof_dir/managed-person.json"
export PEOPLE_MANAGEMENT_ENABLED=true CONTEXT_NOTES_ENABLED=true PARTY_DIRECTORY_ENABLED=true RELATIONSHIPS_ENABLED=true INTERACTIONS_ENABLED=true COMMITMENTS_ENABLED=true PERSON_OVERVIEW_ENABLED=true
docker compose up -d --force-recreate --wait app
uv run python scripts/browser_smoke.py --base-url "http://localhost:$TEST_HTTP_PORT" --people-management on --context-notes on --party-directory on --relationships on --interactions on --commitments on --person-overview on --read-file "$proof_dir/managed-person.json"
